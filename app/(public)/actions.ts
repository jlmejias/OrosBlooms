...
export async function createInquiry(formData: FormData) {
  ...
  for (const [index, file] of files.entries()) {
    ...
    const [asset] = await db.insert(mediaAssets).values({ provider: "neon-object-storage-private", providerId: blob.pathname, url: blob.url, bytes: file.size, format: extension, alt: `Referencia privada ${index + 1}`, visibility: "private" }).returning();
