...
export async function createInquiry(formData: FormData) {
  ...
  const uploadDir = path.join(process.cwd(), "storage", "inquiries");
  ...
  for (const [index, file] of files.entries()) {
    ...
    const [asset] = await db.insert(mediaAssets).values({ provider: "private-local", providerId: filename, url: `private:${filename}`, bytes: file.size, format: extension, alt: `Referencia privada ${index + 1}`, visibility: "private" }).returning();
