...
export async function POST(request: Request) {
  ...
  const [asset] = await db.insert(mediaAssets).values({ provider: "cloudflare-r2-private", providerId: blob.pathname, url: blob.url, bytes: file.size, format: extension, alt: `Comprobante SINPE ${reference}`, visibility: "private" }).returning({ id: mediaAssets.id });
