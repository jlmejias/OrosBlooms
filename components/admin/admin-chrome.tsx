...
import { AppstoreOutlined, BgColorsOutlined, CustomerServiceOutlined, FileImageOutlined, FileProtectOutlined, GiftOutlined, HomeOutlined, InboxOutlined, LogoutOutlined, MenuOutlined, PictureOutlined, SettingOutlined, ShoppingOutlined, ShoppingCartOutlined, TagsOutlined, TeamOutlined, ToolOutlined } from "@ant-design/icons";
import { usePathname, useRouter } from "next/navigation";
import { useState } from "react";
import { logoutAdmin } from "@/app/admin/actions";

const navigation = [
  ["Resumen", "/admin", HomeOutlined], ["Identidad", "/admin/identidad", BgColorsOutlined], ["Categorías", "/admin/categorias", TagsOutlined], ["Productos", "/admin/productos", ShoppingOutlined], ["Complementos", "/admin/complementos", GiftOutlined], ["Combos", "/admin/combos", AppstoreOutlined], ["Galería", "/admin/galeria", PictureOutlined], ["Servicios", "/admin/servicios", ToolOutlined], ["Solicitudes", "/admin/solicitudes", InboxOutlined], ["Clientes", "/admin/clientes", TeamOutlined], ["Pedidos", "/admin/pedidos", ShoppingCartOutlined], ["Portada", "/admin/inicio", HomeOutlined], ["Multimedia", "/admin/multimedia", FileImageOutlined], ["SEO y políticas", "/admin/seo-politicas", FileProtectOutlined], ["Configuración", "/admin/configuracion", SettingOutlined],
] as const;
...
export function AdminChrome({ email, children }: { email: string; children: React.ReactNode }) {
  ...
  return <ConfigProvider theme={{ token: { colorPrimary: "#4b5a43", colorInfo: "#4b5a43", borderRadius: 14, fontFamily: "var(--font-body), Arial, sans-serif", colorBgLayout: "#f4f1ea" }, components: { Menu: { darkItemBg: "#263228", darkItemSelectedBg: "#566851", darkSubMenuItemBg: "#263228" } } }}><App><Layout className="admin-ant-shell">
    ...
    <Drawer className="admin-mobile-drawer" title="Administración" placement="left" width="min(86vw, 320px)" open={menuOpen} onClose={() => setMenuOpen(false)}><Menu mode="inline" selectedKeys={[selected]} items={items} onClick={({ key }) => navigate(key)} />{user}</Drawer>
