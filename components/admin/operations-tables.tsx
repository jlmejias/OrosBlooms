...
import { deleteInquiry, deleteOrder, saveOrder, updateInquiry } from "@/app/admin/actions";
...
type Order = { id: string; reference: string; status: string; total: number; customer: string };
...
export function OrderTable({ items }: { items: Order[] }) {
  ...
  const [changes, setChanges] = useState<Record<string, string>>({}); const [savingId, setSavingId] = useState<string>(); const [deletingId, setDeletingId] = useState<string>();
