...
export function OrderTable({ items }: { items: Order[] }) {
  ...
  return <><Toolbar query={query} setQuery={setQuery} placeholder="Buscar pedido o cliente"><Select placeholder="Todos los estados" value={filter} onChange={setFilter} options={orderStatuses} allowClear/></Toolbar><Table rowKey="id" dataSource={data} pagination={{ pageSize: 10, showSizeChanger: true }} columns={[
    ...
    { title: "Estado", render: (_, item) => <Select disabled={savingId === item.id || deletingId === item.id} style={{ minWidth: 150 }} value={changes[item.id] ?? item.status} options={orderStatuses.filter(option=>option.value===item.status||orderTransitions[item.status as keyof typeof orderTransitions].includes(option.value as never))} onChange={value => setChanges(current => ({ ...current, [item.id]: value }))}/> },
    ...
    { title: "Acciones", render: (_, item) => <Space><Tooltip title="Guardar"><Button aria-label={`Guardar pedido ${item.reference}`} shape="circle" type="text" loading={savingId === item.id} disabled={Boolean(deletingId)} icon={<SaveOutlined/>} onClick={() => save(item)}/></Tooltip><Popconfirm title="Eliminar pedido" description="Esta acción no se puede deshacer." okText="Eliminar" cancelText="Cancelar" okButtonProps={{ danger: true, loading: deletingId === item.id }} onConfirm={() => remove(item.id)}><Button aria-label={`Eliminar pedido ${item.reference}`} shape="circle" type="text" danger loading={deletingId === item.id} disabled={Boolean(savingId)} icon={<DeleteOutlined/>}/></Popconfirm></Space> },
  ]}/></>;
}
