...
export function HomeHeader({locale,logoUrl="/orosblooms-logo.png",logoAlt="OrosBlooms"}:{locale:Locale;logoUrl?:string;logoAlt?:string}) {
  ...
  return <header className={`home-header ${scrolled ? "is-scrolled" : ""}`}>
    <div className="home-header-inner">
      ...
      <div className="home-header-actions"><a className="home-search-desktop" href="/buscar" aria-label={locale==="es"?"Buscar flores y ocasiones":"Search flowers and occasions"}><Icon name="search" size={18} /><span>{locale==="es"?"Explorar flores y ocasiones":"Explore flowers and occasions"}</span></a><button className="language-switch" type="button" onClick={()=>{document.cookie=`oros-locale=${locale==="es"?"en":"es"};path=/;max-age=31536000;SameSite=Lax`;window.location.reload()}} aria-label={locale==="es"?"Cambiar a inglés":"Cambiar a español"}>{locale==="es"?"EN":"ES"}</button><a className="home-icon-button" href="/favoritos" aria-label={locale==="es"?"Favoritos":"Favorites"}><Icon name="heart" /></a><span className="home-icon-button home-deferred" aria-label={locale==="es"?"Perfil disponible próximamente":"Profile coming soon"} title={locale==="es"?"Perfil próximamente":"Profile coming soon"}><Icon name="user" /></span><a className="home-icon-button" href="/carrito" aria-label={locale==="es"?"Carrito":"Cart"}><Icon name="bag" /></a></div>
      <a className="home-icon-button home-mobile-bag" href="/carrito" aria-label={locale==="es"?"Carrito":"Cart"}><Icon name="bag" size={22} /></a>
    </div>
